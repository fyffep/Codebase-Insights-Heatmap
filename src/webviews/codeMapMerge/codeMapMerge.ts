...
import { codeMapMergeWebviewPanel } from "../webviewFactory";
...
export function codemapHTML(args: Map<string, vscode.Uri>): string {
  ...
  codeMapMergeWebviewPanel?.webview.postMessage({
    command: "gitHubUrl",
    data: gitUrl,
  });
  //Request entire codebase data
  api.getMergeMapData().then((responseData) => {
    ...
    if (codeMapMergeWebviewPanel) {
      ...
      console.error("codeMapMergeWebviewPanel was undefined");
    }
  });
