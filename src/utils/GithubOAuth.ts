...
export default class GithubOAuth {
  ...
  private constructor() {
    ...
    this.grantType =
      process.env.GRANT_TYPE || "urn:ietf:params:oauth:grant-type:device_code";
  }
   ...
   */
  public async fetchDeviceAndUserCode(): Promise<{
    deviceCode: string;
    userCode: string;
  }> {
    try {
      ...
      const response = await this.axiosInstance.post(
        `/login/device/code?client_id=${this.clientId}&scope=${this.scope}`
      );
  ...
  public async fetchAccessToken(
    ...
    try {
      ...
      const response = await this.axiosInstance.post(
        `/login/oauth/access_token?client_id=${this.clientId}&device_code=${this.deviceCode}&grant_type=${this.grantType}`
      );

      if (response.status !== 200) {
        throw new Error(
          `Failed to get access token. Response status ${response.status} : ${response.statusText}`
        );
      }
  ...
  public async triggerUserAuthDialogs(context: vscode.ExtensionContext) {
    ...
    vscode.window
      .showInformationMessage(
        "Click here to copy your user code to clipboard",
        copyUserCode
      )
      .then(async () => {
    ...
    vscode.window
      .showInformationMessage(
        "Click here to open auth page. Paste your user code on the Github authentication page ",
